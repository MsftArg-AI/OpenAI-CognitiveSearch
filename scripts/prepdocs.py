...
from azure.storage.blob import BlobServiceClient, generate_account_sas, ResourceTypes, AccountSasPermissions, BlobSasPermissions, generate_blob_sas
...
def upload_blobs(blob_client, filename):
    ...
    if os.path.splitext(filename)[1].lower() == ".pdf":
        ...
        pdf_reader = io.BytesIO(blob_content)
        pdf_file = PyPDF2.PdfReader(pdf_reader)
        #reader = PdfReader(blob.name)
        pages = pdf_file.pages
        for i in range(len(pages)):
            filenameSanitized = filename.replace("(", "").replace(")","")
            blob_name = blob_name_from_file_page(filenameSanitized, i)
...
def get_document_text(blob_client, filename):
    offset = 0
    page_map = []
